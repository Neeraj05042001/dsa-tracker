...
const DSA_PATTERNS = [
  "arrays",        "strings",      "hash_map",      "two_pointers",
  "sliding_window","binary_search","stack",         "linked_list",
  "trees",         "graphs",       "dynamic_programming","backtracking",
  "greedy",        "heap",         "trie",          "intervals",
];

const PATTERN_LABELS: Record<string, string> = {
  arrays: "Arrays", strings: "Strings", hash_map: "Hash Map",
  two_pointers: "Two Pointers", sliding_window: "Sliding Window",
  binary_search: "Binary Search", stack: "Stack / Queue",
  linked_list: "Linked List", trees: "Trees", graphs: "Graphs",
  dynamic_programming: "Dynamic Programming", backtracking: "Backtracking",
  greedy: "Greedy", heap: "Heap / PQ", trie: "Trie", intervals: "Intervals",
};
...
function ReadinessGauge({ score, signals }: {
  score: number;
  ...
  const scoreColor = score >= 75 ? "var(--easy)" : score >= 50 ? "var(--medium)" : "var(--hard)";
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
        <div style={{ position: "relative", width: 140, height: 140, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.06em", marginTop: 2 }}>
              / 100
            </span>
          </div>
        </div>
        ...
        {/* Label */}
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)", marginBottom: 4, letterSpacing: "-0.01em" }}>
            {score >= 80 ? "Interview Ready" : score >= 60 ? "On Track" : score >= 40 ? "Needs Work" : "Just Starting"}
          </div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.6 }}>
            Composite score across<br />consistency, difficulty & recall
          </div>
        </div>
      </div>
        ...
        {signals.map((s, i) => (
          <div key={s.label}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
              ...
              <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: s.color, fontWeight: 700 }}>{s.value}%</span>
            </div>
...
function PatternCoverage({ patternCounts, totalProblems }: {
  patternCounts: Record<string, number>; totalProblems: number;
}) {
          ...
          >
            ...
            {isTouched && (
              <motion.div
                ...
                transition={{ duration: 0.7, delay: 0.3 + i * 0.03, ease: [0.22, 1, 0.36, 1] }}
                style={{
                  position: "absolute", bottom: 0, left: 0, height: 2,
                  background: isMastered ? "var(--accent)" : "color-mix(in srgb, var(--accent) 55%, transparent)",
                }}
              />
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{
                ...
                color: isMastered ? "var(--accent)" : isTouched ? "var(--text-secondary)" : "var(--text-muted)",
                lineHeight: 1,
              }}>
                {count}
              </span>
              ...
              {!isTouched && (
                <span style={{ fontSize: 9, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.05em" }}>
                  UNTOUCHED
                </span>
              )}
            </div>
...
export function AnalyticsClient({ problems }: Props) {
  const stats = useMemo(() => {
    ...
    const weeklyDiffMap: Record<string, { easy: number; medium: number; hard: number }> = {};
    for (const wk of weeklyData) weeklyDiffMap[wk.date] = { easy: 0, medium: 0, hard: 0 };
    ...
    const avgConf = withConf.length > 0
      ? withConf.reduce((s, p) => s + (p.confidence ?? 0), 0) / withConf.length
      : 0;
    ...
    const onTime = reviewed.filter(p => {
      ...
      const overdue = (Date.now() - new Date(p.sm2_next_review).getTime()) / 86400000;
      return overdue <= 3;
    });
    const revDiscipline = reviewed.length > 0
    ...
    const signals = [
      { label: "Consistency (last 14d)", value: consistency, color: "var(--accent)" },
      { label: "Difficulty Spread",      value: diffSpread,  color: "var(--medium)" },
      { label: "Avg Confidence",         value: avgConfPct,  color: "var(--easy)" },
      { label: "Revision Discipline",    value: revDiscipline, color: "var(--lc-color)" },
    ];

    // ── Focus recommendation ──
    const touchedPatterns = DSA_PATTERNS.filter(p => (patternCounts[p] ?? 0) > 0);
    const untouched = DSA_PATTERNS.filter(p => (patternCounts[p] ?? 0) === 0);
    const weakest = touchedPatterns.sort((a, b) => (patternCounts[a] ?? 0) - (patternCounts[b] ?? 0))[0];
    ...
    return {
      total, diffCounts, lcCount, cfCount,
      patternCounts, weeklyData, weeklyDiffData,
      readiness, signals,
      weakestPattern: weakest, untouchedCount: untouched.length,
      platformData, diffData,
  ...
  if (!stats || stats.total === 0) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)" }}>No data yet</div>
        <div style={{ fontSize: 13, color: "var(--text-muted)" }}>Solve some problems to see analytics</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div className="card" style={{ gridColumn: "span 2", padding: "24px 28px" }}>
          <div style={{
        ...
        {/* Mini stat: Total */}
        <MiniStat label="Total Solved" value={stats.total} color="var(--accent)" icon="⚡" delay={0.1} />

        {/* Mini stat: Active days */}
        <MiniStat label="Active (14d)" value={stats.activeDays} suffix=" days" color="var(--easy)" icon="🔥" delay={0.15} />

        {/* Mini stat: LC */}
        <MiniStat label="LeetCode" value={stats.lcCount} color="var(--lc-color)" icon="LC" mono delay={0.2} />

        {/* Mini stat: CF */}
        <MiniStat label="Codeforces" value={stats.cfCount} color="var(--cf-color)" icon="CF" mono delay={0.25} />
      </motion.div>
      ...
      >
        ...
        <div className="card" style={{ padding: "20px 24px" }}>
          <SectionHeader label="Solves by Week" sublabel="last 12 weeks, by difficulty" />
          <div style={{ height: 180, marginTop: 16 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.weeklyDiffData} barSize={14} barGap={2}>
                <XAxis
                  dataKey="label" tick={{ fontSize: 9, fill: "var(--text-muted)", fontFamily: "var(--font-mono)" }}
                  axisLine={false} tickLine={false}
                />
                <YAxis hide allowDecimals={false} />
                <Tooltip content={<ChartTooltip />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
                <Bar dataKey="easy"   stackId="a" fill="var(--easy)"   radius={[0,0,0,0]} name="Easy" />
                <Bar dataKey="medium" stackId="a" fill="var(--medium)" radius={[0,0,0,0]} name="Medium" />
                <Bar dataKey="hard"   stackId="a" fill="var(--hard)"   radius={[2,2,0,0]} name="Hard" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <DonutCard
            ...
            data={stats.diffData}
            total={stats.diffCounts.easy + stats.diffCounts.medium + stats.diffCounts.hard}
          />
          <DonutCard
      ...
      >
        <SectionHeader label="Solve Velocity" sublabel="cumulative solves per week" />
        <div style={{ height: 160, marginTop: 16 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={stats.weeklyData}>
              ...
              />
              <YAxis hide allowDecimals={false} />
              <Tooltip content={<ChartTooltip />} cursor={{ stroke: "var(--border-mid)", strokeWidth: 1 }} />
              <Area
                type="monotone" dataKey="count" name="Solves"
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          ...
          {stats.weakestPattern && (
            <div style={{
              ...
              <span style={{ fontSize: 11, color: "var(--medium)", fontWeight: 600 }}>
                Focus: {PATTERN_LABELS[stats.weakestPattern]}
              </span>
            </div>
          )}
        </div>
        <PatternCoverage patternCounts={stats.patternCounts} totalProblems={stats.total} />
      </motion.div>
...
function MiniStat({ label, value, color, icon, suffix = "", mono = false, delay = 0 }: {
  label: string; value: number; color: string; icon: string;
  suffix?: string; mono?: boolean; delay?: number;
}) {
  return (
    <motion.div
      className="card"
    ...
    >
      ...
      <div style={{
        ...
        {value}<span style={{ fontSize: 14, color: "var(--text-muted)", fontWeight: 500 }}>{suffix}</span>
      </div>
    </motion.div>
  );
}

function DonutCard({ label, data, total }: {
  label: string; data: { name: string; value: number; color: string }[]; total: number;
}) {
  return (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, flex: 1 }}>
          {data.map(d => (
            <div key={d.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", fontWeight: 700, color: d.color }}>
