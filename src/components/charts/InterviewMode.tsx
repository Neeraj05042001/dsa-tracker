...
function getDailyTarget(daysLeft: number, overdueCount: number, totalProblems: number): number {
  ...
  // Base: aim to review all flagged + maintain SM2
...
function getOnTrackStatus(readinessScore: number, daysLeft: number): {
  status: "on_track" | "at_risk" | "critical";
  label: string;
  color: string;
} {
  const requiredGrowthPerDay = daysLeft > 0 ? (80 - readinessScore) / daysLeft : 0;
...
// ─── Collapsed banner (prompt to set date) ───────────────────────────────────
...
function ActiveMode({
  ...
  const { status, label, color } = getOnTrackStatus(readinessScore, days);
    ...
    return (
      <div style={{
        ...
        <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
          Interview date passed. How did it go?
        </span>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: color, opacity: 0.06, filter: "blur(30px)", pointerEvents: "none",
      }} />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 10, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 3 }}>
            days left
          </span>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600, color: "var(--text-muted)" }}>
                Daily target
              </span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>
                {dailyTarget} review{dailyTarget !== 1 ? "s" : ""}
              </span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600, color: "var(--text-muted)" }}>
              ...
              <span style={{
                ...
                color: stats.needs_revision_count > 0 ? "var(--hard)" : "var(--easy)",
              }}>
