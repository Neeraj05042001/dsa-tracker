...
export async function scrapeUserGroups(
  ...
  try {
    ...
    for (let i = 0; i < groupCodes.length; i++) {
      ...
      try {
        const contests = await scrapeGroupContests(code, jsessionid, userSubmissions)
...
function resolveStatus(
  ...
  const matching = submissions.filter(
    s => String(s.contestId) === String(contestId) &&
...
function parseProblemList(
  ...
  const pattern = /href="\/group\/[A-Za-z0-9]+\/contest\/\d+\/problem\/([A-Z][0-9]?)"[^>]*>([^<]+)<\/a>/g
  const seen = new Set<string>()
...
async function cfFetch(url: string, jsessionid: string, attempt = 1): Promise<string> {
  try {
    ...
    if (error.message === 'RATE_LIMITED' || error.message === 'SESSION_EXPIRED') {
      throw error
    }
    // Network error — retry
    if (attempt <= MAX_RETRIES) {
